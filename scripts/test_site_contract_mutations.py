...
CASES = [
    ...
    # DSI-WEB-GOV-1 retargeted this, it did not delete it. Its anchor was the paragraph
    # that claimed no override existed, and that paragraph is gone because the claim
    # became false. The rule it tests is unchanged: a BLOCKING claim is still forbidden,
    # and implementing an override did not license one. Only the phrase "unless
    # overridden" was released, so the mutation now relies on "blocks" alone to fire.
    ("claim: a blocking claim appears with override language", "demo.html",
     'Alongside the comparison status, DSI assigns an <strong>evidence disposition</strong>',
     'DSI blocks invalid comparisons unless overridden. DSI assigns an <strong>evidence disposition</strong>'),
    # --- DSI-WEB-GOV-1: the governed-visible account. The override is real now, so the
    # failure modes are no longer "claiming it exists" but misdescribing what it does.
    ("override: claimed to confer comparability", "demo.html",
     'An override changes what you may conclude, never what was measured, and it does not make the two measurements comparable.',
     'An override makes the two measurements comparable.'),
    ("override: claimed to be automatic", "demo.html",
     'An override requires a non-empty reason, is never automatic',
     'DSI automatically overrides inadmissible results, and requires no reason'),
    ("override: reason requirement dropped", "demo.html",
     'An override requires a non-empty reason',
     'No reason is required to override'),
    ("override: status-preservation clause removed", "demo.html",
     'and <strong>the comparison status does not move</strong>: it is still <span class="mono">NOT COMPARABLE</span>, for the same reasons.',
     'and the comparison is resolved.'),
    ("override: the disposition itself disappears", "demo.html",
     'The comparison above is <span class="mono">INADMISSIBLE</span>.',
     'The comparison above is fine.'),
    ("override: an approval chain is claimed", "demo.html",
     'the override authority is whoever can run the command',
     'an override requires manager approval'),
    ("availability: a2 is claimed generally available", "demo.html",
     'This page describes what the implementation does.',
     'DSI 1.0.0a2 is now generally available.'),
    ("availability: a2 download offered", "demo.html",
     'This page describes what the implementation does.',
     'Download DSI 1.0.0a2 from the link below.'),
    ("states: UNKNOWN collapsed into NOT COMPARABLE", "demo.html",
     '<span class="mono">COMPARABILITY UNKNOWN</span> is <span class="mono">ADMISSIBILITY UNKNOWN</span>',
     '<span class="mono">COMPARABILITY UNKNOWN</span> is <span class="mono">INADMISSIBLE</span>'),
